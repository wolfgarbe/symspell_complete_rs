...
impl SymSpell {
    ...
    /// Input as &str, uses internal whitespace tokenization.
    ///
    ...
    ///
    /// ```
    /// use symspell_complete_rs::{SymSpell};
    /// use std::path::Path;
    ///
    /// let mut symspell: SymSpell = SymSpell::new(2, None,7, 1);
    /// symspell.load_dictionary(Path::new("data/frequency_dictionary_en_82_765.txt"), 0, 1, " ");
    ...
    /// Find suggested spellings for a multi-word input string (supports word splitting/merging).
    /// Returns a list of Suggestion representing suggested correct spellings for the input string.
    /// Input as vector of &str to allow for external tokenization.
    ///
    /// lookup_compound supports compound aware automatic spelling correction of multi-word input strings with three cases:
    /// 1. mistakenly inserted space into a correct word led to two incorrect terms
    /// 2. mistakenly omitted space between two correct words led to one incorrect combined term
    ...
    /// let mut symspell: SymSpell = SymSpell::new(2, None,7, 1);
    /// symspell.load_dictionary(Path::new("data/frequency_dictionary_en_82_765.txt"), 0, 1, " ");
    /// symspell.lookup_compound_vec(&vec!["whereis".to_string(), "th".to_string(), "elove".to_string()], 2, &None,false);
    /// ```
    pub fn lookup_compound_vec(
        &self,
        input_vec: &[String],
        edit_distance_max: usize,
        term_length_threshold: &Option<Vec<usize>>,
        ...
        for (i, term) in input_vec.iter().enumerate() {
            ...
            if i > 0 && !last_combi {
                let mut suggestions_combi: Vec<Suggestion> = self.lookup(
                    &[input_vec[i - 1].clone(), input_vec[i].clone()].join(""),
                    Verbosity::Top,
                    edit_distance_max,
                    term_length_threshold,
                    None,
                ...
                if !suggestions_combi.is_empty() {
                    ...
                    let best2 = if !suggestions.is_empty() {
                        ...
                        Suggestion::new(
                            ...
                            (10f64 / 10usize.saturating_pow(len(&input_vec[i]) as u32) as f64)
                                as usize,
                        )
                    };
            ...
            if !suggestions.is_empty()
                && ((suggestions[0].distance == 0) || (len(&input_vec[i]) == 1))
            {
                ...
                let term_length = len(&input_vec[i]);
                if term_length > 1 {
                    for j in 1..term_length {
                        let part1 = slice(&input_vec[i], 0, j);
                        let part2 = slice(&input_vec[i], j, term_length);
                                    ...
                                    match self.bigrams.get(&*suggestion_split.term) {
                                        ...
                                        Some(&bigram_frequency) => {
                                            ...
                                            if !suggestions.is_empty() {
                                                ...
                                                if suggestion_split.term == input_vec[i] {
                                            ...
                                            } else if suggestion_split.term == input_vec[i] {
                                                cmp::max(
                                                    bigram_frequency,
                                                    cmp::max(
