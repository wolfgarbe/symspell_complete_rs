...
mod tests {
    ...
    use crate::{
        PruningRadixTrie, SymSpell, Verbosity,
    ...
    fn test_lookup_compound_vec() {
        ...
        let typo = ["whereis".to_string(), "th".to_string(), "elove".to_string()].to_vec();
        let correction = "where is the love";
        let results = symspell.lookup_compound_vec(&typo, edit_distance_max, &None, false);
        assert_eq!(1, results.len());
        assert_eq!(correction, results[0].term);
        assert_eq!(2, results[0].distance);
        assert_eq!(585, results[0].count);
    ...
    #[test]
    fn test_pruning_radix_trie() {
        let mut pruning_radix_trie = PruningRadixTrie::new();
        let _ = pruning_radix_trie.load_completions(
            Path::new("./data/frequency_dictionary_en_82_765.txt"),
            0,
            1,
            " ",
        );
        assert_eq!(82_834, pruning_radix_trie.len());

        let input = "predic";
        let expected_completion = "predicted";

        let completions = pruning_radix_trie.lookup_completions(input, None);
        assert_eq!(20, completions.len());

        assert_eq!(expected_completion, completions[0].term.clone());
    }
}
